...
capturaEventos(window, 'load', function(evt){
	...
	capturaEventos(a, 'click', function(evt){
			...
			if(xmlhttp.readyState === 4 && xmlhttp.status === 200) {
				...
				for( var propriedade in dadosJSON ){
					...
					if (typeof dadosJSON[propriedade] !== 'object') {
						...
						// adiciona na div
						div.innerHTML += '<tr><td>' + dadosJSON[propriedade].patientID + '</td><td>'+ dadosJSON[propriedade].nome + '</td><td>'+ dadosJSON[propriedade].numero + '</td><td>'+ dadosJSON[propriedade].tipoExame + '</td><td>'+ dadosJSON[propriedade].modalidade + '</td><td>'+ dadosJSON[propriedade].data + '</td><td> Ler questionário</td></tr>';
					}
				}				
			}
		}
		...
		xmlhttp.open('GET', 'banco/data.json', true);
